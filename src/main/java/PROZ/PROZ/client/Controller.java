 ...
 * 
 * @author Bartosz Ciu�kowski
 * @version 1.0
 */
public class Controller {
	private RestModel model = new RestModel();
    ...
    /**
	 * Puts digits onto the display also prevents from creating numbers like
	 * "000000" or "00000.5".
	 * 
	 * @param specified digit
	 */
    private void setDigit(String dig) {
    	if(checkInfinity(display) == 1) {
    		return;
    	}
    	if(afterEquals) {
    		display.setText("");
    ...
    private void setOperator(String op) {
    	...
    	if(checkPointAtEnd(display) == 0 && checkInfinity(display) == 0) {
    ...
    private void clickEquals() {
    	...
    	if(checkPointAtEnd(display) == 0 && checkInfinity(display) == 0 && !exprToCalc.isEmpty()) {
    		String tmp = display.getText();
    		System.out.println(exprToCalc.concat(tmp));
    		
    		if(checkZeroDivision(exprToCalc.concat(tmp)) == 1) {
    			return;
    		}
	 ...
	 */
    private int checkZeroDivision(String expr) {
    	if(exprToCalc.length() < 3) return 0;
	 ...
	 * 		   1 if there is an error
	 */
    private int checkPointAtEnd(TextField display) {
	 ...
	 * 		   1 if there is an error
	 */
    private int checkInfinity(TextField display) {
